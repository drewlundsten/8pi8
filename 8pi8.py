...
tabControl.add(tab3, text="switch")
...
ttk.Label(tab1, text="V1_8", font=fontStyle).grid(row=1, column=0)
ttk.Label(tab1, text="V2_5", font=fontStyle).grid(row=2, column=0)
ttk.Label(tab1, text="V1_2", font=fontStyle).grid(row=3, column=0)
ttk.Label(tab1, text="LA", font=fontStyle).grid(row=4, column=0)
ttk.Label(tab1, text="TIA", font=fontStyle).grid(row=5, column=0)
ttk.Label(tab1, text="PD", font=fontStyle).grid(row=6, column=0)
ttk.Label(tab1, text="BG", font=fontStyle).grid(row=7, column=0)
ttk.Label(tab1, text="DRV", font=fontStyle).grid(row=8, column=0)
ttk.Label(tab1, text="LED", font=fontStyle).grid(row=9, column=0)
ttk.Label(tab1, text="BF RX", font=fontStyle).grid(row=10, column=0)
ttk.Label(tab1, text="BF TX", font=fontStyle).grid(row=11, column=0)

ttk.Label(tab1, text="Clock", font=fontStyle).grid(row=12, column=0)

pi = pigpio.pi() 
bus = smbus.SMBus(1)
bus2 = smbus.SMBus(2)
# Pi 4 will need this one instead
#bus2 = smbus.SMBus(4)
...
def init():
    ...
    """
    # Explicitly set and drive GPIO on Pi header, although exist resistors
    pi.write(EN_12, 0)
    pi.set_mode(EN_12, pigpio.OUTPUT)

    pi.write(EN_2V5, 0)
    pi.set_mode(EN_2V5, pigpio.OUTPUT)

    pi.write(OUT_EN, 0)
    pi.set_mode(OUT_EN, pigpio.OUTPUT)    

    pi.write(IN_EN, 0)
    pi.set_mode(IN_EN, pigpio.OUTPUT)    

    pi.write(ASIC_EN, 0)
    pi.set_mode(ASIC_EN, pigpio.OUTPUT)
    
    pi.write(ALT_EN, 0)
    pi.set_mode(ALT_EN, pigpio.OUTPUT)

    # Toggle GPIO_RST# to ensure expander pins are in default state
    pi.write(GPIO_RSTN, 0)
    pi.set_mode(GPIO_RSTN, pigpio.OUTPUT)
    sleep(0.1)
    pi.write(GPIO_RSTN, 1)
    

    # set expander port 0 all low to disable LDOs
    bus.write_byte_data(GPIOADDR, 0x02, 0x00) 
    
    # set expander port 1 to enable AN0, deassert RESETB and ARST#             
    bus.write_byte_data(GPIOADDR, 0x03, 0x1c) 
    
    # config expander port 0 as outputs
    bus.write_byte_data(GPIOADDR, 0x06, 0x00) 
    
    # config expander port 1 as outputs
    bus.write_byte_data(GPIOADDR, 0x07, 0x00) 
    ...
    # init AN0
    chip_select(0x10)
    bus.write_i2c_block_data(ADCADDR, 0x05, [0x00, 0xff]) # all DAC (SET pins)
    bus.write_i2c_block_data(ADCADDR, 0x0b, [0x02, 0x00]) # enable VREF
    bus.write_i2c_block_data(ADCADDR, 0x03, [0x01, 0x10]) # enable ADC buffer, Vdac = x2

    # init AN1
    chip_select(0x20)
    bus.write_i2c_block_data(ADCADDR, 0x04, [0x00, 0xff]) # all ADC (voltage sense)
    bus.write_i2c_block_data(ADCADDR, 0x0b, [0x02, 0x00]) # enable VREF
    bus.write_i2c_block_data(ADCADDR, 0x03, [0x01, 0x00]) # enable ADC buffer

    # init AN2
    chip_select(0x40)
    bus.write_i2c_block_data(ADCADDR, 0x04, [0x00, 0xff]) # all ADC (current sense)
    bus.write_i2c_block_data(ADCADDR, 0x0b, [0x02, 0x00]) # enable VREF
    bus.write_i2c_block_data(ADCADDR, 0x03, [0x01, 0x00]) # enable ADC buffer

    # init AN3
    chip_select(0x80)
    bus.write_i2c_block_data(ADCADDR, 0x04, [0x00, 0xff]) # enable all ADC
    bus.write_i2c_block_data(ADCADDR, 0x05, [0x00, 0x03]) # 1..0 are DAC (outputs)
    bus.write_i2c_block_data(ADCADDR, 0x0b, [0x02, 0x00]) # enable VREF
    bus.write_i2c_block_data(ADCADDR, 0x03, [0x01, 0x00]) # enable ADC buffer
...
class PowerSupply:
    ...
    Args:
        ...
        dacbits: bits to write to select trimming dac
    """
    def __init__(self, voltage, status, slope, offset, dacbits = 0x10):
        self.voltage = voltage
        self.status = status
        self.slope = slope
        self.offset = offset 
        self.dacbits = dacbits
...
def chip_select(address):
    ...
    """
    master_bits = bus.read_byte_data(GPIOADDR, 0x03)
    master_bits = master_bits & 0x0f
    master_bits = master_bits | address
    bus.write_byte_data(GPIOADDR, 0x03, master_bits)
...
button_V1_8.grid(row=1, column=3)
    
V1_8_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
V1_8_voltage_entry.grid(row=1, column=1)
...
V2_5_submit.grid(row=2, column=4)

calibrate(V2_5, 0x9ff, 0xfff, 2.776, 3.626)

status_1V2 = False
def isClicked1V2(button):
    global status_1V2
    status_1V2 = not status_1V2
    if status_1V2:
        pi.write(EN_12, 1)
        button["text"] = "ON"
    else:
...
# INIT 1.2V
V1_2 = PowerSupply(1.2, False, 0, 0, 0x80)
V1_2_text = "1.2V"
button_V1_2 = tk.Button(tab1, text="OFF", font=fontStyle,
    command=lambda:isClicked1V2(button_V1_2))
button_V1_2.grid(row=3, column=3)
    
V1_2_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
V1_2_voltage_entry.grid(row=3, column=1)
V1_2_voltage_entry.insert(0, 1.2)
    
V1_2_submit = tk.Button(tab1, text="Submit", font=fontStyle,
    command=lambda:submit(V1_2, V1_2_voltage_entry, V1_2_text, 0x10, "8"))
V1_2_submit.grid(row=3, column=4)

calibrate(V1_2, 0x500, 0xfff, 1.082, 1.579)
...
button_TIA.grid(row=5, column=3) #ON/OFF button, triggers isClicked method

TIA_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
TIA_voltage_entry.grid(row=5, column=1) #user input entry
...
TIA_submit.grid(row=5, column=4) #submit button used to grab component information 

calibrate(TIA, 0x480, 0xa80, 1.653, 2.353)
...
button_PD.grid(row=6, column=3)
    
PD_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
PD_voltage_entry.grid(row=6, column=1)
...
button_BG.grid(row=7, column=3)
    
BG_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
BG_voltage_entry.grid(row=7, column=1)
...
button_DRV.grid(row=8, column=3)
    
DRV_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
DRV_voltage_entry.grid(row=8, column=1)
...
DRV_submit.grid(row=8, column=4)
...
button_LED.grid(row=9, column=3)
    
LED_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
LED_voltage_entry.grid(row=9, column=1)
...
LED_submit.grid(row=9, column=4)
...
# INIT BF_RX
BFR = PowerSupply(1.0, False, 0, 0)
BFR_text = "BF_RX"
button_BFR = tk.Button(tab1, text="OFF", font=fontStyle,
    command=lambda:isClicked(button_BFR, BFR_text, BFR, 0x02, BFR_bits))
button_BFR.grid(row=10, column=3)
    
BFR_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
BFR_voltage_entry.grid(row=10, column=1)
BFR_voltage_entry.insert(0, 1.0)
    
BFR_submit = tk.Button(tab1, text="Submit", font=fontStyle,
    command=lambda:submit(BFR, BFR_voltage_entry, BFR_text, 0x13, "b"))
BFR_submit.grid(row=10, column=4)

calibrate(BFR, 0x4ff, 0xfff, 1.143, 2.002)

    
# INIT BF_TX
BFT = PowerSupply(1.0, False, 0, 0, 0x80)
BFT_text = "BF_TX"
button_BFT = tk.Button(tab1, text="OFF", font=fontStyle,
    command=lambda:isClicked(button_BFT, BFT_text, BFT, 0x03, BFT_bits))
button_BFT.grid(row=11, column=3)
    
BFT_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
BFT_voltage_entry.grid(row=11, column=1)
BFT_voltage_entry.insert(0, 1.0)
    
BFT_submit = tk.Button(tab1, text="Submit", font=fontStyle,
    command=lambda:submit(BFT, BFT_voltage_entry, BFT_text, 0x11, "9"))
BFT_submit.grid(row=11, column=4)

calibrate(BFT, 0x480, 0xd00, 0.925, 1.256)
...
TIA_ADC_voltage_entry.grid(row=5, column=6)

PD_ADC_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
PD_ADC_voltage_entry.grid(row=6, column=6)

BG_ADC_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
BG_ADC_voltage_entry.grid(row=7, column=6)

DRV_ADC_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
DRV_ADC_voltage_entry.grid(row=8, column=6)

LED_ADC_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
LED_ADC_voltage_entry.grid(row=9, column=6)

BFR_ADC_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
BFR_ADC_voltage_entry.grid(row=10, column=6)

BFT_ADC_voltage_entry = tk.Entry(tab1, width=5, font=fontStyle)
BFT_ADC_voltage_entry.grid(row=11, column=6)
...
def adc_voltage_init(channel, entry, text, change):
    ...
    if(text.__eq__("LED") or text.__eq__("1_2") or text.__eq__("BFT")):
        chip_select(0x80)
    else:
        chip_select(0x20)                                     # Set address A0_AN1 to talk to U25
    # bus.write_i2c_block_data(ADCADDR, 0x0b, [0x02, 0x00])    # Enable reference
    #bus.write_i2c_block_data(ADCADDR, 0x04, [0x00, change])    # Set all pins as ADC
    bus.write_i2c_block_data(ADCADDR, 0x02, [0x00, channel]) # X channel for conversion
    sleep(0.01)
    val = bus.read_word_data(ADCADDR, 0x40)                  # Read single channel and convert to voltage
...
read18_voltage_button.grid(row=1, column=7)
...
read25_voltage_button.grid(row=2, column=7)

read12_voltage_button = tk.Button(tab1, text="read", font=fontStyle,
    command=lambda: adc_voltage_init(0x04, ADC_1_2_voltage_entry, "1_2", 0xff))
read12_voltage_button.grid(row=3, column=7)
...
tia_read_voltage_button.grid(row=5, column=7)
...
pd_read_voltage_button.grid(row=6, column=7)
...
bg_read_voltage_button.grid(row=7, column=7)
...
drv_read_voltage_button.grid(row=8, column=7)
...
LED_read_voltage_button.grid(row=9, column=7)

bfr_read_voltage_button = tk.Button(tab1, text="read", font=fontStyle,
    command=lambda: adc_voltage_init(0x08, BFR_ADC_voltage_entry, "BFR", 0xff))
bfr_read_voltage_button.grid(row=10, column=7)

bft_read_voltage_button = tk.Button(tab1, text="read", font=fontStyle,
    command=lambda: adc_voltage_init(0x40, BFT_ADC_voltage_entry, "BFT", 0xff))
bft_read_voltage_button.grid(row=11, column=7)
...
ADC_1_8_current_entry.grid(row=1, column=8)

ADC_2_5_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
ADC_2_5_current_entry.grid(row=2, column=8)

ADC_1_2_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
ADC_1_2_current_entry.grid(row=3, column=8)
...
TIA_ADC_current_entry.grid(row=5, column=8)

PD_ADC_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
PD_ADC_current_entry.grid(row=6, column=8)

BG_ADC_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
BG_ADC_current_entry.grid(row=7, column=8)

DRV_ADC_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
DRV_ADC_current_entry.grid(row=8, column=8)

LED_ADC_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
LED_ADC_current_entry.grid(row=9, column=8)

BFR_ADC_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
BFR_ADC_current_entry.grid(row=10, column=8)

BFT_ADC_current_entry = tk.Entry(tab1, width=8, font=fontStyle)
BFT_ADC_current_entry.grid(row=11, column=8)
...
read18_current_button.grid(row=1, column=9)
...
read25_current_button.grid(row=2, column=9)

read12_current_button = tk.Button(tab1, text="read", font=fontStyle,
    command=lambda: adc_current_init(0x40, ADC_1_2_current_entry, "1.2V", 0xff))
read12_current_button.grid(row=3, column=9)
...
tia_read_current_button.grid(row=5, column=9)
...
pd_read_current_button.grid(row=6, column=9)
...
bg_read_current_button.grid(row=7, column=9)
...
drv_read_current_button.grid(row=8, column=9)
...
LED_read_current_button.grid(row=9, column=9)

bfr_read_current_button = tk.Button(tab1, text="read", font=fontStyle,
    command=lambda: adc_current_init(0x08, BFR_ADC_current_entry, "BFR", 0xff))
bfr_read_current_button.grid(row=10, column=9)

bft_read_current_button = tk.Button(tab1, text="read", font=fontStyle,
    command=lambda: adc_current_init(0x08, BFT_ADC_current_entry, "BFT", 0xff))
bft_read_current_button.grid(row=11, column=9)
...
def swinit(button):
    ...
    if swinit_status:
        ...
        sleep(0.25)
        sw_bus(IN)
        for sw in sw_addr: bus.write_byte_data(sw, 0x00, 0x01)
        sw_bus(OUT)
        for sw in sw_addr: bus.write_byte_data(sw, 0x00, 0x01)
        sleep(0.25)
        ...
        for sw in sw_addr:
            for port in [0xa0, 0xa8, 0xb0, 0xb8]: bus.write_byte_data(sw, port, 0x70)
        ...
        for sw in sw_addr: bus.write_byte_data(sw, 0x40, 0x47)
        # Update
        for sw in sw_addr: bus.write_byte_data(sw, 0x41, 0x01)
        button["text"] = "ON"
    else:
